 ...
 * @version    58ab7bdc89b3d4cd894ef7d853bcc6f0c4dcca6b, v246 (xcart_4_6_2), 2014-02-03 17:25:33, func.cart.php, aim
 ...
 */
function func_restore_serialized_cart($serialized_cart) { // {{{
    global $cart;
    x_session_register('cart');

    if (
        !empty($serialized_cart)
...
function func_calculate_single($cart, $products, $user, $login_type, $provider_for = '')
{
    ...
    if (!empty($active_modules['XPayments_Subscriptions'])) {
        list($_products, $total_fee) = func_xps_calculateProducts($_products);

        $total += $total_fee;
        $subtotal += $total_fee;
        $display_subtotal += $total_fee;
        $discounted_subtotal += $total_fee;
        $display_discounted_subtotal += $total_fee;
    }
