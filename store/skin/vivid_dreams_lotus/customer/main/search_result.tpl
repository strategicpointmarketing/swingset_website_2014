{*
b36a187642e76239b9988efb4d6806ea2e7efaf3, v5 (xcart_4_6_0), 2013-04-17 10:30:14, search_result.tpl, aim
...
//]]>
      ...
      <table cellspacing="0" cellpadding="0" class="width-100" summary="{$lng.lbl_search_products|escape}">
        <tbody>
          <tr>
            ...
            <td class="data-input pattern"><input type="text" name="posted_data[substring]" value="{$search_prefilled.substring|escape}" class="for_s input-field" /></td>
          ...
          <tbody id="adv_search_box"{if not $search_prefilled.need_advanced_options} style="display: none;"{/if}>
            ...
            {if $config.Search_products.search_products_category eq 'Y'}
              <tr>
                ...
                <td class="data-input" colspan="2">
                  <select name="posted_data[categoryid]" class="adv-search-select input-field">
                    <option value="">&nbsp;</option>
            ...
            {if $active_modules.Manufacturers and $manufacturers ne '' and $config.Search_products.search_products_manufacturers eq 'Y'}
              ...
              <tr>
                ...
                <td class="data-input" colspan="2">
                  <select name="posted_data[manufacturers][]" multiple="multiple" class="input-field" size="{if $smarty.section.mnf.total gt 5}5{else}{$smarty.section.mnf.total}{/if}">
            ...
            {if $config.Search_products.search_products_price eq 'Y'}
              <tr>
                ...
                <td colspan="2">
                  <input type="text" size="10" maxlength="15" name="posted_data[price_min]" value="{$search_prefilled.price_min|escape}" class="input-field" />
                  &nbsp;-&nbsp;
                  <input type="text" size="10" maxlength="15" name="posted_data[price_max]" value="{$search_prefilled.price_max|escape}" class="input-field" />
                </td>
              </tr>
            {/if}
            ...
            {if $config.Search_products.search_products_weight eq 'Y'}
              <tr>
                ...
                <td colspan="2">
                  <input type="text" size="10" maxlength="10" name="posted_data[weight_min]" value="{$search_prefilled.weight_min|escape}" class="input-field" />
                  &nbsp;-&nbsp;
                  <input type="text" size="10" maxlength="10" name="posted_data[weight_max]" value="{$search_prefilled.weight_max|escape}" class="input-field" />
                </td>
              </tr>
            {/if}
  ...
  {/capture}
  {include file="customer/dialog.tpl" title=$lng.lbl_search_products content=$smarty.capture.dialog additional_class="adv-search" noborder=true title_page='category'}
...
{if $mode eq "search" and $products ne ""}
  ...
  {/capture}
  {include file="customer/dialog.tpl" title=$lng.lbl_products content=$smarty.capture.dialog sort=true additional_class="products-dialog" title_page='category'}
