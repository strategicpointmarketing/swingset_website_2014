...
{if not $static}
  ...
  <input type="text" id="{$id|escape}" class="zipcode{if $zip_section} {$zip_section}{/if} form-input gd-full gt-full gm-full" name="{$name|escape}" size="32" maxlength="32" value="{$val|escape}" />
  ...
  {strip}
    ...
    <span id="{$zip4id}_container">
      &nbsp;-&nbsp;
      <input type="text" id="{$zip4id}" class="form-input zip4" name="{$zip4name}" size="10" maxlength="4" value="{$zip4|escape}" />
    </span>
  {/strip}
  {/if}
