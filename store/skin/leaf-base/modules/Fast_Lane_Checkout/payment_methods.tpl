{*
0dc73d81aee77c686e41f0ae3a32a207269ca805, v4 (xcart_4_6_1), 2013-09-12 09:04:33, payment_methods.tpl, random 
...
{foreach from=$payment_methods item=payment name=pm}
  ...
  <tr{interline name=pm}{if $payment.is_cod eq "Y"} id="cod_tr{$payment.paymentid}"{/if}{if $payment.processor eq 'ps_paypal_bml.php'} class="paypal-bml-method"{/if}>
    <td>
        flag1
...
<ul class="unstyled">
    {foreach from=$payment_methods item=payment name=pm}
    <li><input class="inline-selector" type="radio" name="paymentid" id="pm{$payment.paymentid}" value="{$payment.paymentid}"{if $payment.is_default eq "1"} checked="checked"{/if} /><span class="semibold secondary-font minion-text">{$payment.payment_method}</span></li>
    {/foreach}
</ul>