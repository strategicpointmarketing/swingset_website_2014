...
<div class="gd-row gt-row">
    ...
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="new-line capitalize" for="{$id_prefix}title">{$lng.lbl_title}</label></div>
    <span{if $default_fields.title.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span>
    {include file="main/title_selector.tpl" val=$address.titleid id="`$id_prefix`title" name="`$name_prefix`[title]"}
</div>
{/if}
...
{if $default_fields.firstname.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}firstname">{$lng.lbl_first_name}<span{if $default_fields.firstname.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
        <input class="form-input gd-full gt-full gm-full" type="text" id="{$id_prefix}firstname" name="{$name_prefix}[firstname]" size="32" maxlength="128" value="{$address.firstname|default:$personal_firstname|escape}" autofocus />
    </div>
{/if}

{if $default_fields.lastname.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}lastname">{$lng.lbl_last_name}<span{if $default_fields.lastname.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
...
{if $default_fields.address.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}address">{$lng.lbl_address}<span{if $default_fields.address.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
...
{if $default_fields.address_2.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}address_2">{$lng.lbl_address_2}<span{if $default_fields.address_2.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
...
{if $default_fields.city.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}city">{$lng.lbl_city}<span{if $default_fields.city.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
        <input class="form-input gd-full gt-full gm-full" type="text" id="{$id_prefix}city" name="{$name_prefix}[city]" size="32" maxlength="64" value="{$address.city|default_cond:$config.General.default_city:$use_default_addr|escape}" {if $is_popup_estimate_shipping}autofocus{/if} />
    </div>
{/if}

{if $default_fields.county.avail eq 'Y' and $config.General.use_counties eq "Y"}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}county">{$lng.lbl_county}<span{if $default_fields.county.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
        {include file="main/counties.tpl" counties=$counties name="`$name_prefix`[county]" id="`$id_prefix`county" default=$address.county country_name="`$id_prefix`country"}
    </div>
{/if}
...
{if $default_fields.state.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}state">{$lng.lbl_state}<span{if $default_fields.state.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
        {include file="main/states.tpl" states=$states name="`$name_prefix`[state]" default=$address.state|default:$config.General.default_state default_country=$address.country|default:$config.General.default_country id="`$id_prefix`state" country_name="`$id_prefix`country"}
    </div>
{/if}
...
{if $default_fields.country.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}country">{$lng.lbl_country}<span{if $default_fields.country.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
...
{if $default_fields.state.avail eq 'Y' and $default_fields.country.avail eq 'Y'}
    <div style="display: none;">
      <span{if $default_fields.state.required eq 'Y'} class="data-required"{else} class="data-optional"{/if}>
        {include file="main/register_states.tpl" state_name="`$name_prefix`[state]" country_name="`$id_prefix`country" county_name="`$name_prefix`[county]" state_value=$address.state|default:$config.General.default_state county_value=$address.county}
    </div>
{/if}
...
{if $default_fields.zipcode.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}zipcode">{$lng.lbl_zip_code}<span{if $default_fields.zipcode.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
        {include file="main/zipcode.tpl" zip_section=$zip_section name="`$name_prefix`[zipcode]" id="`$id_prefix`zipcode" val=$address.zipcode|default_cond:$config.General.default_zipcode:$use_default_addr zip4=$address.zip4}
    </div>
{/if}
...
{if $default_fields.phone.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}phone">{$lng.lbl_phone}<span{if $default_fields.phone.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
...
{if $default_fields.fax.avail eq 'Y'}
    <div class="gd-half gd-columns gt-half gt-columns">
        <label class="form-label new-line capitalize" for="{$id_prefix}fax">{$lng.lbl_fax}<span{if $default_fields.fax.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
...
{if $additional_fields ne ''}
    ...
    {foreach from=$additional_fields item=v}
        ...
        {if $v.section eq 'B' and $v.avail eq 'Y'}
            ...
            <div class="gd-half gd-columns gt-half gt-columns">
                <label for="additional_values{if $address_type ne ''}_{$address_type}{/if}_{$v.fieldid}">{$v.title}<span{if $v.required eq 'Y'} class="data-required">*{else} class="data-optional">&nbsp;{/if}</span></label>
                ...
                {elseif $v.type eq 'S'}
                    <select class="gd-full gt-full gm-full mvxs" name="additional_values[{$v.fieldid}]{if $address_type ne ''}[{$address_type}]{/if}" id="additional_values{if $address_type ne ''}_{$address_type}{/if}_{$v.fieldid}">
                        {foreach from=$v.variants item=o}
                            <option value='{$o|escape}'{if ($address_type ne '' and $v.value.$address_type eq $o) or ($v.value eq $o)} selected="selected"{/if}>{$o|escape}</option>
                        {/foreach}
                    </select>
