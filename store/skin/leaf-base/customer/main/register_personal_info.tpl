{*
c3f2c63d6ec12340c11b3f3d5d6a44e80a517ccb, v2 (xcart_4_4_5), 2011-12-20 09:58:43, register_personal_info.tpl, aim
...
vim: set ts=2 sw=2 sts=2 et:
*}
<!-- Begin skin/leaf-base/customer/main/register_personal_info.tpl -->
{if $is_areas.P eq 'Y'}
...
<ul class="gd-row gt-row unstyled">
    ...
    {if $default_fields.firstname.avail eq 'Y'}
        <li class="gd-half gd-columns gt-half gt-columns mtxs">
            <label class="form-label" for="firstname">{$lng.lbl_first_name}<span{if $default_fields.firstname.required eq 'Y'} class="data-required">*{else}>&nbsp;{/if}</span></label>
            <input class="form-input gd-10of12 gt-8of9 gm-full" type="text" id="firstname" name="firstname" size="32" maxlength="128" value="{$userinfo.firstname|escape}" />
        </li>
    {/if}
    {if $default_fields.lastname.avail eq 'Y'}
        <li class="gd-half gd-columns gt-half gt-columns mtxs">
            <label class="form-label" for="lastname">{$lng.lbl_last_name}<span{if $default_fields.lastname.required eq 'Y'} class="data-required">*{else}>&nbsp;{/if}</span></label>
            <input class="form-input gd-10of12 gt-8of9 gm-full" type="text" id="lastname" name="lastname" size="32" maxlength="128" value="{$userinfo.lastname|escape}" />
        </li>
    {/if}
    {if $default_fields.company.avail eq 'Y'}
        <li class="gd-half gd-columns gt-half gt-columns mtxs">
            <label class="form-label" for="company">{$lng.lbl_company}<span{if $default_fields.company.required eq 'Y'} class="data-required">*{else}>&nbsp;{/if}</span></label>
            <input class="form-input gd-10of12 gt-8of9 gm-full" type="text" id="company" name="company" size="32" maxlength="255" value="{$userinfo.company|escape}" />
        </li>
    {/if}
    {if $default_fields.url.avail eq 'Y'}
        <li class="gd-half gd-columns gt-half gt-columns mtxs">
            <label class="form-label" for="url">{$lng.lbl_web_site}<span{if $default_fields.url.required eq 'Y'} class="data-required">*{else}>&nbsp;{/if}</span></label>
            <input class="form-input gd-10of12 gt-8of9 gm-full" type="text" id="url" name="url" size="32" maxlength="128" value="{$userinfo.url|escape}" />
        </li>
    {/if}

    {if $default_fields.tax_number.avail eq 'Y'}
        <li class="gd-half gd-columns gt-half gt-columns mtxs">
            <label class="form-label" for="tax_number">{$lng.lbl_tax_number}<span{if $default_fields.tax_number.required eq 'Y'} class="data-required">*{else}>&nbsp;{/if}</span></label>

            {if $userinfo.tax_exempt ne "Y" or $config.Taxes.allow_user_modify_tax_number eq "Y" or $usertype eq "A" or $usertype eq "P"}
                <input class="form-input gd-10of12 gt-8of9 gm-full" type="text" id="tax_number" name="tax_number" size="32" maxlength="32" value="{$userinfo.tax_number|escape}" />
            {else}
                {$userinfo.tax_number}
            {/if}
        </li>
    ...
    {if $config.Taxes.enable_user_tax_exemption eq 'Y'}
        {if (($userinfo.usertype eq "C" or $smarty.get.usertype eq "C") and $userinfo.tax_exempt eq "Y") or ($usertype eq "A" or $usertype eq "P")}
            <li class="gd-half gd-columns gt-half gt-columns mtxs">
                <label class="form-label" for="tax_exempt">{$lng.lbl_tax_exemption}</label>
                {if $usertype eq "A" or $usertype eq "P"}
                    <input type="checkbox" id="tax_exempt" name="tax_exempt" value="Y"{if $userinfo.tax_exempt eq "Y"} checked="checked"{/if} />
                {elseif $userinfo.tax_exempt eq "Y"}
                    {$lng.txt_tax_exemption_assigned}
    ...
    {if $usertype eq "A" or $usertype eq "P"}
        <li class="gd-half gd-columns gt-half gt-columns mtxs">
            <label class="gd-10of12 gt-8of9" for="referer">{$lng.lbl_referred_by}</label>
            {if $userinfo.referer}
                <a href="{$userinfo.referer}">{$userinfo.referer}</a>
            {else}
                {$lng.lbl_unknown}
...
<!-- End skin/leaf-base/customer/main/register_personal_info.tpl -->