...
<li id="address_box_{$address.id|default:0}" class="address-box{if $address.id eq $current} address-current{/if}{if $mode eq 'select' and not $add_new and $address.id neq $current} address-select cursor-hover pointer{/if}"{if $mode eq 'select' and not $add_new and $type ne '' and $for ne '' and $checkout_module ne 'One_Page_Checkout' or $address.id lte 0} onclick="javascript: $('#address_{$address.id|default:0}').submit();"{/if}>
  <div class="address-bg">
    <div class="address-main">
      ...
      {if $add_new}
          <div class="mbs">
            <a class="form-link capitalize" href="popup_address.php" onclick="javascript: return !popupOpen('popup_address.php{if $mode eq 'select'}?return=select&for={$for}&type={$type}{/if}');">{$lng.lbl_add_new_address}</a>
          </div>

      {else}

        {if $address.default_s eq 'Y' or $address.default_b eq 'Y'}
          <div class="{*address-default*}mvxs bold capitalize">
        ...
        {*<br />*}
        <div class="{*buttons-row buttons-auto-separator*}mvs">
