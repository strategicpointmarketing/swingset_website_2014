...
{if $location}
                    ...
                    {foreach from=$location item=l name=location}
                        {if $l.1 and not $smarty.foreach.location.last and not $smarty.foreach.location.first}
                            <a href="{$l.1|amp}" class="minion-text tertiary-heading bread-crumb{if $smarty.foreach.location.last} last-bread-crumb{/if}{if $smarty.foreach.location.first} first-bread-crumb{/if}">{if $smarty.foreach.location.first}&nbsp;{else}{if $webmaster_mode eq "editor"}{$l.0}{else}{$l.0|escape}{/if}{/if}</a>
                        {else}
