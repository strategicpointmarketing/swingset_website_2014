{*
4182101b6b98229d35c8bcb53c0469e2a06fbe95, v10 (xcart_4_5_4), 2012-10-08 09:50:51, address_fields.tpl, random
...
<tr class="address-row-{$id}"{if $hide} style="display:none;"{/if}>
  ...
  {if $address_fields.state.avail eq 'Y' and $address_fields.country.avail eq 'Y'}
    <tr style="display: none;">
      <td{if $address_fields.state.required eq 'Y'} class="data-required"{/if}>
...
{if $additional_fields ne ''}
  ...
  {foreach from=$additional_fields item=v key=k}
    ...
    {if $v.section eq 'B' and $v.avail eq 'Y'}
      <tr>
        <td class="data-name"><label for="additional_values_{$v.fieldid}_{$id}">{$v.title}</label></td>
        <td{if $v.required eq 'Y'} class="data-required">*{else}>&nbsp;{/if}</td>
        <td>
          {if $v.type eq 'T'}
            <input type="text" name="additional_values[{$v.fieldid}][{$id}]" id="additional_values_{$v.fieldid}_{$id}" size="32" value="{$address_book_additional_fields.$id.$k.value|escape}" />

          {elseif $v.type eq 'C'}
            <input type="checkbox" name="additional_values[{$v.fieldid}][{$id}]" id="additional_values_{$v.fieldid}_{$id}" value="Y"{if $address_book_additional_fields.$id.$k.value eq 'Y'} checked="checked"{/if} />

          {elseif $v.type eq 'S'}
            <select name="additional_values[{$v.fieldid}][{$id}]" id="additional_values_{$v.fieldid}_{$id}">
              {foreach from=$v.variants item=o}
                <option value='{$o|escape}'{if $address_book_additional_fields.$id.$k.value eq $o} selected="selected"{/if}>{$o|escape}</option>
              {/foreach}
            </select>
