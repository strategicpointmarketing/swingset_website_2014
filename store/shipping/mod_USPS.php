 ...
 * @version    58ab7bdc89b3d4cd894ef7d853bcc6f0c4dcca6b, v135 (xcart_4_6_2), 2014-02-03 17:25:33, mod_USPS.php, aim
...
/*
https://www.usps.com/business/web-tools-apis/Rate-Calculators-v1-8a.htm#_Toc363742147
...
function func_USPS_parse_methods($_packages, $intl_use, $_allowed_shipping_methods, $_pack_limit_key, $_usps_options)
{
    ...
    foreach ($_packages as $_package) {
        ...
        foreach ($postages as $p) {
            ...
            if (!floatval($rate))
                $rate = func_array_path($p, ($intl_use) ? "Postage/0/#" : "CommercialRate/0/#");
...
function func_USPS_prepare_rate_query($_pack, $_usps_options, $intl_use, $ZO, $ZD, $dst_country, $is_first_class) { // {{{
    ...
    $_params = @unserialize($_usps_options['param00']);
    if (!empty($_params)) {
        $_usps_options['mailtype'] = $_params['mailtype'];
        $_usps_options['ground_only'] = $_params['ground_only'];
        $_usps_options['selected_services'] = $_params['selected_services'];
    }
    ...
    $_usps_options['selected_services'] = !empty($_usps_options['selected_services']) ? $_usps_options['selected_services'] : array('ALL');
...
EOT;
        ...
        // https://www.usps.com/business/web-tools-apis/Rate-Calculators-v1-8a.htm#OLE_LINK8
        ...
        $common_dim_girth_xml = func_USPS_get_xml_girth($package_size, '', $dim_girth_xml);
        $services = array(
            'STANDARD POST' => array('container' => '', 'machinable' => "<Machinable>$machinable</Machinable>", 'size' => $package_size, '_girth' => $common_dim_girth_xml),
        );
        $services['ALL'] = $services['LIBRARY'] = $services['MEDIA'] = $services['STANDARD POST'];
        $services['STANDARD POST']['ground_only'] = "<GroundOnly>$_usps_options[ground_only]</GroundOnly>";
